...
export const Sidebar = ({ setOpen }: SidebarProps) => {
  ...
  useKeyboardShortcut(["shift", "g"], () => {
    navigate("/dashboard/getmail");
    setOpen?.(false);
  });

  useKeyboardShortcut(["shift", "c"], () => {
    navigate("/dashboard/careerguidance");
    setOpen?.(false);
  });
  ...
  const sidebarItems: SidebarItem[] = [
    ...
    {
      path: "/dashboard/getmail",
      name: t`Get Email`,
      shortcut: "⇧G",
      icon: <Envelope />,
    },
    {
      path: "/dashboard/careerguidance",
      ...
      <div className="ml-12 flex justify-center lg:ml-0">
        <Button asChild size="icon" variant="ghost" className="size-10 p-0">
          <Link to="/">
            ...
            <div className="font-medium text-3xl mx-auto hidden lg:block">InternVista</div>
          </Link>
        </Button>
      </div>
